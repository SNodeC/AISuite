...
if (process.argv.length !== 6) {
    throw new Error("usage: generate-codex2-protocol.mjs SCHEMA PROTOCOL_SOURCE OUTPUT_HEADER OUTPUT_MANIFEST");
}
...
output += `#ifndef AI_OPENAI_CODEX2_GENERATED_PROTOCOLTYPES_H\n#define AI_OPENAI_CODEX2_GENERATED_PROTOCOLTYPES_H\n\n`;
output += `#include <algorithm>\n#include <cstddef>\n#include <cstdint>\n#include <optional>\n#include <string>\n#include <string_view>\n#include <type_traits>\n#include <utility>\n#include <vector>\n#include <nlohmann/json.hpp>\n\n`;
output += `namespace ai::openai::codex2::generated {\n\n`;
...
output += `} // namespace ai::openai::codex2::generated\n\n`;
output += operationMacro("AI_OPENAI_CODEX2_CLIENT_REQUESTS", clientRequests);
output += operationMacro("AI_OPENAI_CODEX2_PARAMETERLESS_CLIENT_REQUESTS", clientRequests.filter((operation) => !operation.paramsRequired));
output += operationMacro("AI_OPENAI_CODEX2_SERVER_REQUESTS", serverRequests);
output += operationMacro("AI_OPENAI_CODEX2_CLIENT_NOTIFICATIONS", clientNotifications);
output += operationMacro("AI_OPENAI_CODEX2_PARAMETERLESS_CLIENT_NOTIFICATIONS", clientNotifications.filter((operation) => !operation.paramsRequired));
output += operationMacro("AI_OPENAI_CODEX2_SERVER_NOTIFICATIONS", serverNotifications);
output += "\n#endif\n";
