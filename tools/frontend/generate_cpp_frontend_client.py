...
def render_metadata(bindings: list[dict[str, Any]], counts: dict[str, int]) -> str:
    """Render the metadata-only authority used before typed facades land."""
    ...
    lines = [
        ...
        "    };",
        "",
        f"    inline constexpr std::array<BindingMetadata, {len(bindings)}> AllBindings{{{{",
    ]
    for binding in bindings:
        lines.append(
...
def main() -> int:
    ...
    rendered = render_metadata(bindings, counts)
