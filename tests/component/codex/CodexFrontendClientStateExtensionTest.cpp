...
namespace {
    ...
    void testExtensionPreservation(tests::support::TestResult& result) {
        ...
        const bool controllerAndSessionPreserved =
            ...
            state.sessions().value->front().extensions.value("vendorSession", "") == "session-extension";
        result.expectTrue(controllerAndSessionPreserved, "controller and session additive fields remain in their typed records");
