...
namespace {
    ...
    class Endpoint final : public codex2::frontend::client::TransportEndpoint {
    public:
        bool send(const nlohmann::json& message) override {
            tests::codex2::traceCommunication("FrontendSdk", "transport-endpoint", "client-to-bridge", "send", message);
            sent.push_back(message);
            return acceptWrites;
        }

        void close(std::string_view reason) noexcept override {
            tests::codex2::traceCommunication("FrontendSdk", "transport-endpoint", "local", "close", {{"reason", reason}});
            ++closeCount;
            closeReason = reason;
        }
...
int main() {
    namespace clientRequests = ai::openai::codex2::generated::client_requests;
    namespace v2 = ai::openai::codex2::generated::v2;
    ...
    sdk.onRawJson([&](auto direction, const nlohmann::json& message) {
        tests::codex2::traceCommunication("FrontendSdk",
                                          "frontend-sdk",
                                          direction == ai::openai::codex2::protocol::AppServerDirection::ToAppServer ? "to-app-server"
                                                                                                                     : "from-app-server",
                                          "raw-json",
                                          message);
        if (direction == ai::openai::codex2::protocol::AppServerDirection::ToAppServer) {
            ++rawOutbound;
        } else {
            ++rawInbound;
        }
    ...
    sdk.onBridgeEvent([&](const nlohmann::json& event) {
        tests::codex2::traceCommunication("FrontendSdk", "frontend-sdk", "from-bridge", "telemetry", event);
        ++bridgeEvents;
    });
    client::ClientConnection connection(
        ...
        {.onConnected =
             [&] {
                 tests::codex2::traceCommunication("FrontendSdk", "client-connection", "lifecycle", "connected");
                 ++connected;
             },
         .onDisconnected =
             [&] {
                 tests::codex2::traceCommunication("FrontendSdk", "client-connection", "lifecycle", "disconnected");
                 ++disconnected;
             },
         .onFailure =
             [&](std::string reason) {
                 tests::codex2::traceCommunication("FrontendSdk", "client-connection", "lifecycle", "failure", {{"reason", reason}});
                 ++failures;
             }});
    Endpoint endpoint;
    ...
    const std::string requestId = sdk.threadList(params, [&](v2::ThreadListResponse& response) {
        tests::codex2::traceCommunication("FrontendSdk", "typed-callback", "from-app-server", "thread/list", response.getRaw());
    ...
    sdk.threadList(params, [&](v2::ThreadListResponse& response) {
        tests::codex2::traceCommunication("FrontendSdk", "typed-callback", "local", "transport-error", response.getRaw());
