...
namespace {
    ...
    struct Scenario {
        explicit Scenario(tests::codex2::TestHarness& test)
            : test(test) {
        }
        ...
        void completeIfReady() {
            ...
            completed = true;
            tests::codex2::traceCommunication("Provider stdio", "scenario", "local", "complete");
...
int main(int argc, char* argv[]) {
    ...
    int eventLoopResult = 1;
    {
        ...
        router.onRawJson([&scenario](codex2::protocol::AppServerDirection direction, const nlohmann::json& message) {
            tests::codex2::traceCommunication("Provider stdio",
                                              "backend-sdk",
        ...
        router.onProviderLifecycle([&](bool connected) {
            tests::codex2::traceCommunication("Provider stdio", "provider-endpoint", "lifecycle", connected ? "connected" : "disconnected");
            if (!connected) {
        ...
        tests::codex2::traceCommunication("Provider stdio", "child-process", "lifecycle", "spawned", {{"pid", endpoint.pid()}});
        ...
        endpoint.stop();
        tests::codex2::traceCommunication("Provider stdio", "child-process", "lifecycle", "stopped");
    }
