...
namespace {
    namespace bridge = ai::openai::codex2::bridge;
    ...
    public:
        ...
        bool send(const nlohmann::json& message) override {
            tests::codex2::traceCommunication("BridgeRouting", label, "bridge-to-frontend", "send", message);
            messages.push_back(message);
            return acceptWrites;
        }

        void close(std::string_view reason) override {
            tests::codex2::traceCommunication("BridgeRouting", label, "bridge-to-frontend", "close", {{"reason", reason}});
            ++closeCount;
            closeReason = reason;
        }
    ...
    public:
        bool send(const nlohmann::json& message) override {
            tests::codex2::traceCommunication("BridgeRouting", "provider", "bridge-to-provider", "send", message);
            messages.push_back(message);
            return acceptWrites;
        }
