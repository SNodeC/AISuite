...
int main() {
    namespace protocol = ai::openai::codex2::protocol;

    tests::codex2::TestHarness test;
    std::vector<nlohmann::json> messages;
    std::vector<std::string> errors;
    const auto onMessage = [&messages](nlohmann::json message) {
        tests::codex2::traceCommunication("JsonLineFramer", "framer", "decoded", "json-message", message);
        messages.push_back(std::move(message));
    };
    const auto onError = [&errors](std::string error) {
        tests::codex2::traceCommunication("JsonLineFramer", "framer", "terminal", "framing-error", {{"reason", error}});
        errors.push_back(std::move(error));
    };

    protocol::JsonLineFramer fragmented(64);
    tests::codex2::traceCommunication("JsonLineFramer", "byte-stream", "inbound", "fragment", {{"bytes", 9}, {"data", "{\"first\":"}});
