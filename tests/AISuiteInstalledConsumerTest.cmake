...
set(installed_aisuite_abi_targets
    "${aisuite_install}/lib/cmake/AISuite/AISuiteTargets-v7.cmake"
)
...
foreach(obsolete_abi IN ITEMS 1 2 3 4 5 6)
    set(obsolete_aisuite_abi_targets
        "${aisuite_install}/lib/cmake/AISuite/AISuiteTargets-v${obsolete_abi}.cmake"
    )
    if(EXISTS "${obsolete_aisuite_abi_targets}" OR
       IS_SYMLINK "${obsolete_aisuite_abi_targets}")
        fail_installed(
            "obsolete AISuiteTargets-v${obsolete_abi}.cmake is installed: ${obsolete_aisuite_abi_targets}"
        )
    endif()
endforeach()
...
foreach(codex_library IN LISTS codex_libraries)
    set(codex_soversion_library
        "${aisuite_install}/lib/lib${codex_library}.so.7"
    )
    if(NOT EXISTS "${codex_soversion_library}")
        fail_installed(
            "installed SOVERSION-7 library is missing: ${codex_soversion_library}"
        )
    endif()
    foreach(obsolete_soversion IN ITEMS 1 2 3 4 5 6)
        set(codex_obsolete_soversion_library
            "${aisuite_install}/lib/lib${codex_library}.so.${obsolete_soversion}"
        )
        if(EXISTS "${codex_obsolete_soversion_library}" OR
           IS_SYMLINK "${codex_obsolete_soversion_library}")
            fail_installed(
                "obsolete SOVERSION-${obsolete_soversion} compatibility library is installed: ${codex_obsolete_soversion_library}"
            )
        endif()
    endforeach()
    execute_process(
    ...
    string(
        ...
        "Library soname: [lib${codex_library}.so.7]"
        codex_soname_index
    )
    if(codex_soname_index EQUAL -1)
        fail_installed(
            "${codex_library} does not declare the required .so.7 SONAME"
        )
    endif()
    if(codex_library STREQUAL "aisuite-openai-codex-frontend")
        ...
        foreach(codex_dynamic_line IN LISTS codex_dynamic_lines)
            if(codex_dynamic_line MATCHES
               "\\(NEEDED\\).*Shared library: \\[libaisuite-openai-codex-frontend-protocol\\.so\\.7\\][ \t]*$"
            )
        ...
        if(NOT frontend_protocol_needed)
            fail_installed(
                "installed frontend DSO does not directly need libaisuite-openai-codex-frontend-protocol.so.7"
            )
        endif()
    endif()
endforeach()
...
if(AISUITE_BUILD_APPS AND AISUITE_BUILD_CODEX_FRONTEND_CLIENT)
    ...
    string(
        ...
        "Shared library: [libaisuite-openai-codex-frontend-client.so.7]"
...
foreach(consumer IN LISTS installed_consumers)
    ...
    if(NOT uses_aisuite EQUAL -1)
        ...
        string(FIND "${linked_libraries}"
               "libaisuite-openai-codex-frontend-protocol.so.1"
               linked_legacy_frontend_protocol
        )
        string(FIND "${linked_libraries}"
        ...
        string(FIND "${linked_libraries}" "libaisuite-openai-codex.so.2"
               linked_previous_main
        )
        string(FIND "${linked_libraries}"
               "libaisuite-openai-codex-backend.so.2"
               linked_previous_backend
        )
        string(FIND "${linked_libraries}"
               "libaisuite-openai-codex-frontend-protocol.so.2"
               linked_previous_frontend_protocol
        )
        string(FIND "${linked_libraries}"
               "libaisuite-openai-codex-frontend.so.2"
               linked_previous_frontend
        )
        string(FIND "${linked_libraries}"
               "libaisuite-openai-codex-frontend-client.so.2"
               linked_previous_frontend_client
        )
        string(FIND "${linked_libraries}" "libaisuite-openai-codex.so.3"
               linked_immediately_previous_main
        )
        string(FIND "${linked_libraries}"
               "libaisuite-openai-codex-backend.so.3"
               linked_immediately_previous_backend
        )
        string(FIND "${linked_libraries}"
               "libaisuite-openai-codex-frontend-protocol.so.3"
               linked_immediately_previous_frontend_protocol
        )
        string(FIND "${linked_libraries}"
               "libaisuite-openai-codex-frontend.so.3"
               linked_immediately_previous_frontend
        )
        string(FIND "${linked_libraries}"
        ...
        string(FIND "${linked_libraries}"
               ...
               linked_prior_backend
        )
        string(FIND "${linked_libraries}"
               "libaisuite-openai-codex-frontend-protocol.so.5"
               linked_prior_frontend_protocol
        )
        string(FIND "${linked_libraries}"
               "libaisuite-openai-codex-frontend.so.5"
               linked_prior_frontend
        )
        string(FIND "${linked_libraries}"
               "libaisuite-openai-codex-frontend-client.so.5"
               linked_prior_frontend_client
        )
        string(FIND "${linked_libraries}" "libaisuite-openai-codex.so.6"
               linked_latest_previous_main
        )
        string(FIND "${linked_libraries}"
               "libaisuite-openai-codex-backend.so.6"
               linked_latest_previous_backend
        )
        string(FIND "${linked_libraries}"
               "libaisuite-openai-codex-frontend-protocol.so.6"
               linked_latest_previous_frontend_protocol
        )
        string(FIND "${linked_libraries}"
               "libaisuite-openai-codex-frontend.so.6"
               linked_latest_previous_frontend
        )
        string(FIND "${linked_libraries}"
               "libaisuite-openai-codex-frontend-client.so.6"
               linked_latest_previous_frontend_client
        )
        if(NOT linked_legacy_main EQUAL -1 OR
           ...
           NOT linked_legacy_frontend_client EQUAL -1 OR
           NOT linked_previous_main EQUAL -1 OR
           NOT linked_previous_backend EQUAL -1 OR
           NOT linked_previous_frontend_protocol EQUAL -1 OR
           NOT linked_previous_frontend EQUAL -1 OR
           NOT linked_previous_frontend_client EQUAL -1 OR
           NOT linked_immediately_previous_main EQUAL -1 OR
           ...
           NOT linked_superseded_frontend_client EQUAL -1 OR
           NOT linked_prior_main EQUAL -1 OR
           NOT linked_prior_backend EQUAL -1 OR
           NOT linked_prior_frontend_protocol EQUAL -1 OR
           NOT linked_prior_frontend EQUAL -1 OR
           NOT linked_prior_frontend_client EQUAL -1 OR
           NOT linked_latest_previous_main EQUAL -1 OR
           NOT linked_latest_previous_backend EQUAL -1 OR
           NOT linked_latest_previous_frontend_protocol EQUAL -1 OR
           NOT linked_latest_previous_frontend EQUAL -1 OR
           NOT linked_latest_previous_frontend_client EQUAL -1)
            fail_installed(
                "${consumer} resolved an obsolete AISuite Codex .so.1/.so.2/.so.3/.so.4/.so.5/.so.6 runtime"
            )
        endif()
        string(FIND "${linked_libraries}" "libaisuite-openai-codex.so.7"
               linked_main
        )
        string(FIND "${linked_libraries}"
               "libaisuite-openai-codex-backend.so.7"
               linked_backend
        )
        string(FIND "${linked_libraries}"
               "libaisuite-openai-codex-frontend.so.7"
               linked_frontend
        )
        string(FIND "${linked_libraries}"
               "libaisuite-openai-codex-frontend-client.so.7"
    ...
    if(consumer STREQUAL "AISuiteInstalledCodexFrontendProtocolConsumer")
        ...
        endforeach()
        string(FIND "${linked_libraries}"
               "libaisuite-openai-codex-frontend-protocol.so.7"
...
if(NOT saw_aisuite_main_runtime_library OR
   ...
   NOT saw_snodec_runtime_library)
    fail_cross_repo(
        "consumer runtime proof did not observe every configured AISuite Codex .so.7 library and SNode.C from their isolated install prefixes"
    )
endif()
