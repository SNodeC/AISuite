...
#if defined(AISUITE_CODEX_TEST_TLS)
...
namespace {
    ...
    client::ClientConnection makeConnection(Scenario& scenario, frontend::CodexBridge& sdk) {
        return client::ClientConnection(
            ...
            {.onConnected =
                 [&scenario] {
                     tests::codex::traceCommunication(name(scenario.transport), "client-connection", "lifecycle", "connected");
                     ++scenario.connected;
                 },
             .onDisconnected =
                 [&scenario] {
                     tests::codex::traceCommunication(name(scenario.transport), "client-connection", "lifecycle", "disconnected");
    ...
    int runNetwork(char* argv[],
        ...
        int eventLoopResult = 1;
        {
            ...
            Server server("codex-transport-server", router, std::size_t{MaximumFrameBytes});
            Client configuredClient("codex-transport-client", connection, std::size_t{MaximumFrameBytes});
    ...
    int runUnix(char* argv[],
        ...
        int eventLoopResult = 1;
        {
            ...
            Server server("codex-unix-transport-server", router, std::size_t{MaximumFrameBytes});
            Client configuredClient("codex-unix-transport-client", connection, std::size_t{MaximumFrameBytes});
            server.getConfig()->Instance::forceUnrequired();
            configuredClient.getConfig()->Instance::forceUnrequired();
            configuredClient.getConfig()->setRetry(false);
            server.getConfig()->Connection::setMaximumWriteQueueBytes(MaximumWriteQueueBytes);
