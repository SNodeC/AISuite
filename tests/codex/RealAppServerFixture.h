...
#ifndef TESTS_CODEX_REALAPPSERVERFIXTURE_H
#define TESTS_CODEX_REALAPPSERVERFIXTURE_H
...
namespace tests::codex {
    ...
    public:
        ...
        RealAppServerFixture(ai::openai::codex::bridge::CodexBridge& bridge,
    ...
    private:
        static std::string makeCodexHome() {
            ...
            const std::filesystem::path path =
                ...
                ("codex-real-appserver-" + std::to_string(::getpid()) + '-' + std::to_string(++counter));
            std::filesystem::create_directories(path);
            return path.string();
        }

        static ai::openai::codex::provider::StdioAppServerOptions options(std::string executable,
                                                                           const std::string& codexHome,
                                                                           std::function<void(int)> onExit) {
            ai::openai::codex::provider::StdioAppServerOptions result;
        ...
        void initialize() {
            using Initialize = ai::openai::codex::generated::client_requests::Initialize;
            Initialize::Params parameters({
                {"clientInfo", {{"name", "codex_transport_test"}, {"title", "Codex Transport Test"}, {"version", "1"}}},
        ...
        ai::openai::codex::provider::StdioAppServer endpoint_;
