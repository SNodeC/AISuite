...
namespace {
    ...
    struct Scenario {
        Scenario(tests::codex::TestHarness& test, Transport transport)
            : test(test)
            , transport(transport) {
        }
        ...
        void completeIfReady() {
            ...
            completed = true;
            tests::codex::traceCommunication(transportName(transport), "scenario", "local", "complete");
    ...
    private:
        void onConnected() override {
            tests::codex::traceCommunication(transportName(scenario_.transport), "fake-app-server", "lifecycle", "websocket-connected");
        }
        ...
        void onDisconnected() override {
            tests::codex::traceCommunication(transportName(scenario_.transport), "fake-app-server", "lifecycle", "websocket-disconnected");
        }
    ...
    int runLoopback(char* argv[], tests::codex::TestHarness& test, Transport transport, StartTransport startTransport) {
        ...
        int eventLoopResult = 1;
        {
            ...
            WebApp webApp("codex-provider-websocket-server");
            ...
            HttpClient httpClient("codex-provider-websocket-client", beginUpgrade, endHttp, endpoint.state());
