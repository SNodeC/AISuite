...
The typed `UserMessageThreadItem` separately retains typed content. Its
normalized snapshot/event projection preserves the complete ordered textual
presentation up to Codex's aggregate 1,048,576-Unicode-scalar input limit,
including empty multipart positions. Non-text input and additional entry
metadata are omitted rather than copied as opaque frontend data. The adjacent
`textTruncated`, `contentTruncated`, `originalContentBytes`,
`retainedContentBytes`, `originalContentItems`, and `retainedContentItems`
fields distinguish incomplete text from omitted non-text or metadata. The byte
counts are compact serialized array sizes, so an empty retained array counts as
two bytes. This payload-specific accounting does not alter the top-level
`contentTruncated` or `droppedContentBytes`; those fields retain their
accumulated-visible-content meaning for every item type.
