...
filesystem, reviews, and approvals typed protocol surface. The protocol pin
remains upstream tag `rust-v0.144.6`.

The vendored schemas and Rust protocol source define the protocol surface.
`ProtocolSurfaceRegistryData.inc` records current implementation state, and
the generated descriptors, production codecs, fixtures, and current product
tests validate the typed behavior. No separate audit or closure report is a
...
The current public-header policy, isolated header self-containment test,
installed consumer, and package tests verify the supported public surface.
They validate current build and consumption behavior rather than treating a
historical symbol or layout snapshot as an active authority.
...
The current registry keeps all 68 A1.3 identities Complete. Generated
descriptors, production codecs, schema-derived fixtures, public-header and
package checks, and focused wire/lifecycle tests verify every root, response,
and union alternative without experimental leakage or frontend/backend semantic
expansion. All five approval request types use the shared occurrence/generation
path concurrently over AF_UNIX, with out-of-order responses retaining their
original request IDs and response schemas and with no cross-delivery.
`serverRequest/resolved` reconciliation remains separate from direct response
delivery.
