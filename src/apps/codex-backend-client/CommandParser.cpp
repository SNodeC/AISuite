...
namespace apps::codex_backend_client {
    ...
    namespace {
        ...
        bool parseThreadOption(std::string_view option,
            ...
            if (!known) {
                // Parser failures are presentation-safe diagnostics. Never
                // mirror an unreviewed token: command lines may contain
                // credentials, prompts, paths, or other sensitive values.
                error = "unknown thread option";
                return false;
            }
    ...
    ParsedCommand CommandParser::parse(std::string_view line) {
        ...
        if (name == "reconnect") {
            return remainder.empty() ? ParsedCommand{ReconnectCommand{}} : ParsedCommand{usageError("usage: reconnect")};
        }
        if (name == "watch") {
            const std::string_view value = takeWord(remainder);
        ...
        return usageError("unknown command; enter 'help' for available commands");
    }
