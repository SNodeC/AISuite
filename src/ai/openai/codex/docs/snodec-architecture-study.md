# SNode.C Architecture Study for codex
...
transport interaction is necessary, the associated `SocketConnection`. For a
WebSocket application, the corresponding learning surface adds
`SubProtocolFactory` and `SubProtocol`. Beyond these classes, an application
author primarily needs their callbacks and public methods. Address-family,
transport, TLS, HTTP, and WebSocket composition remains framework/build-system
work. SNode.C therefore targets ease of application use, not triviality of its
...
### 1.6 Consequence for codex configuration

The `codex` application `Configuration` should contain only semantics not
...
`codex` should consume this mechanism for network provider/client modes rather
...
The agreed `codex` architecture does not add bearer-token or other
application authentication. This does not remove transport configuration or
TLS functionality. It means `codex` does not create a parallel frontend
...
WebSocket transmitter, but `codex` must work correctly with the current API
...
For `codex`, factories should construct only endpoint adapters. The central
...
For `codex`, `CodexBridge` similarly coordinates endpoints, but its domain is
deliberately narrow:
...
The canonical implementation was completed after this study and audited
against these findings. The resulting source applies the following rules:
...
The following rules remain binding for continued implementation:
...
Appropriate codex timer uses are provider restart delay, frontend reconnect
...
`QueueResult`, the boolean is still a hard delivery boundary. codex must not
...
It is not a multi-subscriber fanout mechanism. The codex bridge must perform
frontend fanout explicitly at the protocol-envelope level.

### 15.5 codex provider consequence
...
is appropriate only if codex later needs an explicitly approved streaming
...
This client machinery is useful architectural evidence, but codex is not an
...
socket source still has a finite queue, so codex must surface a rejected frame
...
The codex `CodexBridge` therefore remains an application-owned coordinator.

### 18.5 codex WebSocket contract
...
TLS remains below HTTP and WebSocket. WSS is thus the same codex subprotocol
...
This reinforces the codex requirement that every app-server JSON-RPC message
...
codex should copy the composition pattern, not the semantic ownership:
...
close phase, and diagnostic context. This is an intentional codex requirement,
...
codex frontend context/factory should be instantiated for each desired family
...
the codex application `SubCommand`.
...
For codex, IPv4 is one frontend listener option. It does not imply a different
...
codex must let the native instance configuration decide whether an IPv6
...
the transport implementation and codex must not add its own unconditional
socket-path removal.
...
codex protocol's perspective, this is another ordered connection carrying
...
Like L2CAP, RFCOMM has legacy and TLS compositions. codex must not implement a
...
The detailed subsystem study narrows the codex implementation as follows:
...
For the criteria relevant to codex--modular transport composition, separation
