# MQTTSuite Architecture Study for codex
...
implemented codex build and CI use that branch directly.
...
10. The principal codex lesson is the shape of composition, not reuse of MQTT
    domain state. codex should copy factories, adapters, shared-coordinator
...
MQTT. It does not justify a second semantic state authority in codex.
...
the clearest reusable pattern for codex stream and WebSocket frontends.
...
database persistence in the bridge. codex must leave retained Codex data in
...
## 12. Detailed codex Guidance

### 12.1 Patterns to adopt

1. Create one thin codex protocol adapter per provider or frontend endpoint,
...
5. Do not infer send success merely because a protocol serializer or
   WebSocket convenience method returned. codex still requires explicit
...
### 12.3 Recommended codex object correspondence
...
codex design with a stateless frontend proxy SDK rather than another stateful
...
The correct adaptation is narrower than MQTTSuite's applications: codex needs
...
Against the codex-relevant criteria of modular transport composition,
