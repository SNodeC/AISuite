...
required codex2 frontend proxy SDK. It preserves the familiar command-line
...
Codex client transport adapters belong to the codex2 library so CodexUI and
other remote SNode.C applications can reuse them:

```text
ai/openai/codex2/frontend/client/
    ClientConnection
    StreamSocketContext
    StreamSocketContextFactory
    WebSocketSubProtocol
...
snapshot machinery, and synchronization barriers are not reusable codex2
components.
...
The branch was initially prepared with this bootstrap scope:
...
- add `src/apps/codex-bridge-client`
- add this architecture contract
- do not delete old files
- do not modify the OpenAI app-server

The focused codex2 tests below do not reactivate any legacy Codex test target.

## Implemented System Report

The initial codex2 implementation is complete for the stateless bridge scope
...
The reusable static library is built as `ai-openai-codex2`, exported and
installed as `AISuite::OpenAICodex2`. It contains:
...
`tools/generate-codex2-protocol.mjs`. Its adjacent manifest records the exact
...
`ai::openai::codex2::bridge::CodexBridge`. Its important hand-written entry
points are:
...
```cpp
using namespace ai::openai::codex2;
...
`ai::openai::codex2::frontend::CodexBridge`. It intentionally mirrors all of
...
relevant application and reusable codex2 library. **Conditional** means the
...
|---|---|---|---|---|---|---|
| `AISuite::OpenAICodex2` backend SDK/router | Through `AppServerEndpoint` | Through `FrontendEndpoint` adapters | not applicable | Complete imported schema | Complete | None |
...
| `AISuite::OpenAICodex2` frontend proxy SDK | remote through bridge | not applicable | Through `ClientConnection` adapters | Complete imported schema | Complete | None |
...
|---|---|---|---|---|
| Protocol utility | JSONL framing | `Codex2JsonLineFramerTest` | not applicable | Complete and passing |
| Frontend proxy SDK + `ClientConnection` | in-memory endpoint boundary | `Codex2FrontendSdkTest` | covered through every real frontend transport below | Complete and passing |
| Backend SDK/router | in-memory provider/frontend endpoints | `Codex2BridgeRoutingTest` | covered through every real frontend transport below | Complete and passing |
| `codex-bridge` provider | owned stdio JSONL | `Codex2Provider_stdio` | exercised by all nine `Codex2RealAppServer_*` cases | Complete and passing |
| `codex-bridge` provider | Unix WebSocket | `Codex2Provider_websocket_unix` | no separate automated external-process case | Production path complete; deterministic case passing |
| `codex-bridge` provider | IPv4 WebSocket | `Codex2Provider_websocket_ipv4` | authenticated external app-server run performed manually | Production path complete; deterministic case passing and live path observed |
| `codex-bridge` provider | IPv6 WebSocket | `Codex2Provider_websocket_ipv6` | no separate automated external-process case | Production path complete; deterministic case passing |
| Bridge server + proxy client | Unix JSONL | `Codex2Stream_unix` | `Codex2RealAppServer_Stream_unix` | Complete and passing |
| Bridge server + proxy client | IPv4 JSONL | `Codex2Stream_ipv4` | `Codex2RealAppServer_Stream_ipv4` | Complete and passing |
| Bridge server + proxy client | IPv6 JSONL | `Codex2Stream_ipv6` | `Codex2RealAppServer_Stream_ipv6` | Complete and passing |
| Bridge server + proxy client | IPv4 TLS JSONL | `Codex2Stream_tls_ipv4` | `Codex2RealAppServer_Stream_tls_ipv4` | Complete and passing when TLS is enabled |
| Bridge server + proxy client | IPv6 TLS JSONL | `Codex2Stream_tls_ipv6` | `Codex2RealAppServer_Stream_tls_ipv6` | Complete and passing when TLS is enabled |
| Bridge server + proxy client | IPv4 WebSocket | `Codex2WebSocket_websocket_ipv4` | `Codex2RealAppServer_WebSocket_websocket_ipv4` | Complete and passing when WebSocket is enabled |
| Bridge server + proxy client | IPv6 WebSocket | `Codex2WebSocket_websocket_ipv6` | `Codex2RealAppServer_WebSocket_websocket_ipv6` | Complete and passing when WebSocket is enabled |
| Bridge server + proxy client | IPv4 WSS | `Codex2WebSocket_wss_ipv4` | `Codex2RealAppServer_WebSocket_wss_ipv4` | Complete and passing when TLS/WebSocket are enabled |
| Bridge server + proxy client | IPv6 WSS | `Codex2WebSocket_wss_ipv6` | `Codex2RealAppServer_WebSocket_wss_ipv6` | Complete and passing when TLS/WebSocket are enabled |
...
| TLS/WSS setup | generated loopback certificates | `Codex2GenerateTlsCertificates` | shared by encrypted real cases | Complete and passing |
...
Legacy `ai::openai::codex`, legacy applications, and legacy tests remain in the
repository but are disabled unless explicitly requested through the legacy
build option. `AISUITE_BUILD_CODEX2_TESTS` registers only `tests/codex2`.

The library headers, `AISuite::OpenAICodex2` target, `codex-bridge`, and
...
builds SNode.C `master`/HEAD, configures codex2 with legacy Codex disabled,
builds with parallelism eight, runs the focused tests, and verifies
installation.
...
The codex2 test suite validates the narrow bridge architecture at its actual
...
Only `tests/codex2` is registered when `AISUITE_BUILD_CODEX2_TESTS` is enabled.
The old top-level `tests` hierarchy remains disabled and its existing sources
remain files on disk only. Every codex2 test target is named explicitly; there
is no recursive discovery that could silently add legacy or broad tests.
...
```text
ctest --test-dir /home/voc/projects/drafts/AISuite-extraction/build/codex-build -V -L codex2
```
...
- `trace: "codex2.communication"` as a stable discriminator;
...
native SNode.C SocketClient or HTTP/WebSocket client
    -> codex2 production transport binding
...
native SNode.C SocketServer or Express WebApp
    -> codex2 production stream SocketContext or WebSocket SubProtocol
    -> backend CodexBridge
...
The canonical build currently registers 26 codex2 CTest cases:
...
`codex2` always builds against the current `master`/HEAD of the upstream
...
The installed backend/frontend SDK target is `AISuite::OpenAICodex2`. Its
...
focused codex2 suite against SNode.C `master`/HEAD, runs the registered focused
tests, and verifies installation while legacy Codex remains disabled.
